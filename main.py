...
ax0 = fig.add_subplot(131, projection="3d", title="graphene")  # 3D설정
ax0.view_init(elev=90, azim=0)  # 보는 각도 설정
ax1 = fig.add_subplot(132, title="z-potential")  # z-potential 그래프
ax2 = fig.add_subplot(133, title="x-potential")
...
def potential_3d(cor1, cor2, d_ij=0.205, x_ij=0.4073, sigma=2.5):  # cor1, cor2: 좌표 2개, d: DIJ, x: xIJ, sigma: 몇 시그마까지 할지
    distance = np.sqrt(math.pow((cor1[0]-cor2[0]), 2) + math.pow((cor1[1]-cor2[1]), 2) + math.pow((cor1[2]-cor2[2]), 2))
    if distance < sigma * math.pow(2, -1/6) * x_ij:  # 시그마 안의 거리에 있는 원자는 그냥 포텐셜 값 구하기
        return d_ij * (math.pow((x_ij/distance), 12) - 2 * math.pow((x_ij/distance), 6))
    else:  # 시그마 값 넘는 원자는 potential 0 으로 취급
        return 0


# (0, 0, z)에서 z움직이면서 potential 측정
...
for z in np.arange(0.3, 0.6, 0.001):  # z의 범위[nm]
    ...
    for i in range(len(graphene_xyz)):  # 그래핀 원자 수만큼 반복
        potential_sum += potential_3d(graphene_xyz[i], (0, 0, z))  # i번째 그래핀 원자와 팁원자(0, 0, z)간의 potential을 함수로 구해서 누적
...
print(potential_sum_min, z_potential_min)

# z_potential_min 에서 x 움직이면서 potential 측정, 그래프 그리기
for x in np.arange(0, 1, 0.001):
    potential_sum = 0  # potential의 합 초기화
    for i in range(len(graphene_xyz)):  # 그래핀 원자 수만큼 반복
        potential_sum += potential_3d(graphene_xyz[i], (x, 0, z_potential_min))  # i번째 그래핀 원자와 팁원자(x, 0, z_potential_min)간의 potential을 함수로 구해서 누적
    ax2.scatter(x, potential_sum)  # 세번째 그래프 점 찍기
